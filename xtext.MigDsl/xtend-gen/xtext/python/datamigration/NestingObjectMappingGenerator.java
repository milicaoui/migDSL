...
public class NestingObjectMappingGenerator {
  public String getNestingObjectMappingContent(final Mapping mapping, final RelationalDb relDb) {
    String _xblockexpression = null;
    {
      final NestingObject map = ((NestingObject) mapping);
      final List<String> referencedColumnNames = ConstraintUtils.getColumnNamesForColumns(map.getForeignkey().getRefrenced());
      final List<String> referencingColumnNames = ConstraintUtils.getColumnNamesForColumns(map.getForeignkey().getReferencing());
      ...
      final String input_table_name = Utils.getTableNameFromReferencedColumn(relDb, map.getForeignkey().getReferencing().get(0));
      final String ref_table_name = Utils.getTableNameFromReferencedColumn(relDb, map.getForeignkey().getRefrenced().get(0));
      StringConcatenation _builder = new StringConcatenation();
      _builder.append("# %% Nesting object mapping ");
      {
      ...
      _builder.newLine();
      _builder.append("\t");
      _builder.append("if str(i[0])!=\'None\':");
      _builder.newLine();
      _builder.append("\t\t");
